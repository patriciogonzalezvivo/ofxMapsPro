...
void ofxMapPro::allocate(float _width, float _height){
    width = _width;
    height = _height;
    fbo.allocate(width,height);
}

void ofxMapPro::loadMap(MapProviderRef _provider) {
	setMapProvider( _provider );
}

void ofxMapPro::loadMap(string _predef){
...
void ofxMapPro::setUseTexture(bool bUseTex){
    // ...
}

ofTexture& ofxMapPro::getTextureReference(){
    return fbo.getTextureReference();
}

void ofxMapPro::update(){
    if (bEnable){
...
void ofxMapPro::draw(float _x, float _y,float _width, float _height) {
...
void ofxMapPro::setExtent( const MapExtent &extent, bool forceIntZoom ){
...
void ofxMapPro::setMapProvider( MapProviderRef _mapProvider ){
    images.clear();
    queue.clear();
    recentImages.clear();
    visibleKeys.clear();
...
void ofxMapPro::setCenter(const Coordinate &center) {
    centerCoordinate = center;
}

void ofxMapPro::setCenter(const Location &location) {
...
void ofxMapPro::setZoom(const double &_zoom) {
    centerCoordinate = centerCoordinate.zoomTo(_zoom);
}

MapExtent ofxMapPro::getExtent() const{
...
double ofxMapPro::getZoom() const {
    return centerCoordinate.zoom;
}

Location ofxMapPro::getCenter() const {
	return provider->coordinateLocation( centerCoordinate );
}

//----------------------------------------------------------------- CONVERSION
ofPoint ofxMapPro::coordinatePoint(const Coordinate &target) const {
...
Coordinate ofxMapPro::pointCoordinate(const ofPoint &point) const {
...
ofPoint ofxMapPro::locationPoint(const Location &location) const {
	return coordinatePoint( provider->locationCoordinate(location));
}

Location ofxMapPro::pointLocation(const ofPoint &point) const {
	return provider->coordinateLocation(pointCoordinate(point));
}

void ofxMapPro::panBy(const ofPoint &_pos) {
...
void ofxMapPro::scaleBy(const double &_scale, const ofPoint &_pos) {
...
void ofxMapPro::rotateBy(const double &_radians, const ofPoint &_pos) {
	panBy(_pos*-1.0);
	rotation += _radians;
	panBy(_pos);
}
...
void ofxMapPro::grabTile(Coordinate coord) {
...
void ofxMapPro::tileDone(Coordinate coord, ofImage *img) {
